...
HRESULT stm::CDialogEventHandler::OnTypeChange(IFileDialog *pfd) {
...
HRESULT stm::CDialogEventHandler::OnItemSelected(IFileDialogCustomize *pfdc, DWORD dwIDCtl, DWORD dwIDItem) {
...
HRESULT stm::CDialogEventHandler_CreateInstance(REFIID riid, void **ppv){
	*ppv = NULL;
...
fs::path file_dialog(HWND owner) {
	fs::path file;
...
fs::path folder_dialog(HWND owner) {
	fs::path file;
	// CoCreate the File Open Dialog object.
	IFileDialog *pfd = NULL;
	HRESULT hr = CoCreateInstance(CLSID_FileOpenDialog, NULL, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&pfd));
