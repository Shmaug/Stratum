...
inline void make_triangle_shading_data(inout ShadingData r, const InstanceData instance, const float2 bary, const PackedVertexData v0, const PackedVertexData v1, const PackedVertexData v2) {
	r.uv = v0.uv() + (v1.uv() - v0.uv())*bary.x + (v2.uv() - v0.uv())*bary.y;

	const float3 dPds = instance.transform.transform_vector(v0.position - v2.position);
	...
	if (all(shading_normal.xyz == 0) || any(isnan(shading_normal))) {
		...
		shading_normal = normalize(instance.transform.transform_vector(shading_normal));
...
inline void make_triangle_shading_data_from_barycentrics(out ShadingData r, const InstanceData instance, const uint primitive_index, const float2 bary) {
	...
	make_triangle_shading_data(r, instance, bary, v0, v1, v2);
}
inline void make_triangle_shading_data_from_position(out ShadingData r, const InstanceData instance, const uint primitive_index, const float3 local_position) {
...
inline void make_sphere_shading_data(out ShadingData r, const InstanceData instance, const float3 local_position) {
	const float3 normal = normalize(instance.transform.transform_vector(local_position));
	r.position = instance.transform.transform_point(local_position);
...
inline void make_volume_shading_data(out ShadingData r, const InstanceData instance, const float3 local_position) {
	r.position = instance.transform.transform_point(local_position);
	r.shape_area = 0;
	r.uv_screen_size = 0;
}
...
inline void make_shading_data(out ShadingData r, const uint instance_index, const uint primitive_index, const float3 local_position) {
	if (instance_index == INVALID_INSTANCE) {
		...
		case INSTANCE_TYPE_SPHERE:
			make_sphere_shading_data(r, instance, local_position);
			break;
		case INSTANCE_TYPE_VOLUME:
			make_volume_shading_data(r, instance, local_position);
			break;
		case INSTANCE_TYPE_TRIANGLES:
			make_triangle_shading_data_from_position(r, instance, primitive_index, local_position);
			break;
		}
	}
}
