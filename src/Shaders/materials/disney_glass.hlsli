...
Real disneyglass_eval_reflect(const Real F, const Real D, const Real G, const Real cos_theta_in) {
	return (F * D * G) / (4 * abs(cos_theta_in));
}
Real disneyglass_eval_refract(const Real F, const Real D, const Real G, const Real cos_theta_in, const Real h_dot_in, const Real h_dot_out, const Real local_eta, const bool adjoint) {
	...
	return ((1 - F) * D * G * abs(h_dot_out * h_dot_in)) / (abs(cos_theta_in) * sqrt_denom * sqrt_denom);
}
...
void disneyglass_eval(const DisneyMaterialData bsdf, out MaterialEvalRecord r, const Vector3 dir_in, const Vector3 dir_out, const bool adjoint) {
    ...
    Vector3 h = reflect ? normalize(dir_in + dir_out) : normalize(dir_in + dir_out * local_eta);
	...
	if (reflect) {
		r.f = bsdf.base_color() * disneyglass_eval_reflect(F, D, G, dir_in.z);
		r.pdf_fwd = disneyglass_reflect_pdf(F, D, G_in, dir_in.z);
		r.pdf_rev = disneyglass_reflect_pdf(F, D, G_out, dir_out.z);
	} else {
		const Real h_dot_out = dot(h, dir_out);
		r.f = sqrt(bsdf.base_color()) * disneyglass_eval_refract(F, D, G, dir_in.z, h_dot_in, h_dot_out, local_eta, adjoint);
		r.pdf_fwd = disneyglass_refract_pdf(F, D, G_in, dir_in.z, h_dot_in, h_dot_out, local_eta);
		r.pdf_rev = disneyglass_refract_pdf(F, D, G_out, dir_out.z, h_dot_out, h_dot_in, 1/local_eta);
	}
}

void disneyglass_sample(const DisneyMaterialData bsdf, out MaterialSampleRecord r, const Vector3 rnd, const Vector3 dir_in, inout Spectrum beta, const bool adjoint) {
	...
	if (h_dot_out_sq <= 0 || rnd.z <= F) {
		...
		r.pdf_fwd = disneyglass_reflect_pdf(F, D, G_in, dir_in.z);
		r.pdf_rev = disneyglass_reflect_pdf(F, D, G_out, r.dir_out.z);
		r.eta = 0;

		beta *= disneyglass_eval_reflect(F, D, G_in * G_out, dir_in.z) / r.pdf_fwd;
	} else {
		// Refraction
		...
		r.pdf_fwd = disneyglass_refract_pdf(F, D, G_in, dir_in.z, h_dot_in, h_dot_out, local_eta);
		r.pdf_rev = disneyglass_refract_pdf(F, D, G_out, r.dir_out.z, h_dot_out, h_dot_in, 1/local_eta);
		r.eta = local_eta;

		beta *= disneyglass_eval_refract(F, D, G_in * G_out, dir_in.z, h_dot_in, h_dot_out, local_eta, adjoint) / r.pdf_fwd;
	}
