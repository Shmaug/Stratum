...
#include "../common/pt_descriptors.hlsli"
...
#include "../common/bdpt_connect.hlsli"
...
void sample_visibility(uint3 index : SV_DispatchThreadID) {
	...
	if (gPathVertex.instance_index() != INVALID_INSTANCE) {
		...
		const float3 prevCamPos = tmul(gPrevViews[view_index].world_to_camera, gInstanceMotionTransforms[gPathVertex.instance_index()].motion_transform).transform_point(gPathShadingData.position);
