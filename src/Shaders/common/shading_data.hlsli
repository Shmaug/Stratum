...
void make_triangle_shading_data(inout ShadingData r, const TransformData transform, const float2 bary, const PackedVertexData v0, const PackedVertexData v1, const PackedVertexData v2) {
...
void make_triangle_shading_data(out ShadingData r, const InstanceData instance, const TransformData transform, const uint primitive_index, const float2 bary) {
	const uint3 tri = load_tri(gSceneParams.gIndices, instance, primitive_index);
	const PackedVertexData v0 = gSceneParams.gVertices[tri.x];
	const PackedVertexData v1 = gSceneParams.gVertices[tri.y];
	const PackedVertexData v2 = gSceneParams.gVertices[tri.z];
...
void make_triangle_shading_data_from_position(out ShadingData r, const InstanceData instance, const TransformData transform, const uint primitive_index, const float3 local_position) {
	r.position = transform.transform_point(local_position);
	const uint3 tri = load_tri(gSceneParams.gIndices, instance, primitive_index);
	const PackedVertexData v0 = gSceneParams.gVertices[tri.x];
	const PackedVertexData v1 = gSceneParams.gVertices[tri.y];
...
void make_sphere_shading_data(out ShadingData r, const InstanceData instance, const TransformData transform, const float3 local_position) {
...
void make_volume_shading_data(out ShadingData r, const InstanceData instance, const TransformData transform, const float3 local_position) {
	r.position = transform.transform_point(local_position);
	r.shape_area = 0;
	r.uv_screen_size = 0;
}

void make_shading_data(out ShadingData r, const uint instance_index, const uint primitive_index, const float3 local_position) {
	if (instance_index == INVALID_INSTANCE) {
		...
		case INSTANCE_TYPE_SPHERE:
			make_sphere_shading_data(r, instance, gSceneParams.gInstanceTransforms[instance_index], local_position);
			break;
		case INSTANCE_TYPE_VOLUME:
			make_volume_shading_data(r, instance, gSceneParams.gInstanceTransforms[instance_index], local_position);
			break;
		case INSTANCE_TYPE_TRIANGLES:
			make_triangle_shading_data(r, instance, gSceneParams.gInstanceTransforms[instance_index], primitive_index, local_position.xy);
			break;
		}
	}
}
