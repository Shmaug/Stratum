...
struct RoughDielectric {
    ...
    inline BSDFEvalRecord eval(const Vector3 dir_in, const Vector3 dir_out, const PathVertexGeometry vertex) {
        ...
        const Spectrum Kt = vertex.eval(specular_transmittance);
        // Clamp roughness to avoid numerical issues.
        const Real rgh = clamp(vertex.eval(roughness), gMinRoughness, 1);
    ...
    inline Spectrum eval_albedo  (const PathVertexGeometry vertex) { return vertex.eval(specular_reflectance); }
