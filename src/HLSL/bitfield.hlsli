...
#define BF_SET_UNORM(x, f, start, len) BF_SET(x, (uint)(f * (float)SET_LSBITS(len)), start, len)
...
inline uint pack_normal_octahedron(const float3 v) {
	...
	return pack_f16_2((v.z <= 0) ? ((1 - abs(p.yx)) * lerp(-1, 1, p >= 0)) : p);
}

inline min16float3 unpack_normal_octahedron(const uint packed_nrm) {
	const float2 p = unpack_f16_2(packed_nrm);
	float3 v = float3(p, 1 - dot(1, abs(p)));
	if (v.z < 0) v.xy = (1 - abs(v.yx)) * lerp(-1, 1, v.xy >= 0);
	return (min16float3)normalize(v);
}
