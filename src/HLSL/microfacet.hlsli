...
inline float schlick_fresnel(const float F0, const float cos_theta) {
    return F0 + (1 - F0) * pow(max(1 - cos_theta, 0.f), 5);
}
inline float3 schlick_fresnel(const float3 F0, const float cos_theta) {
    return F0 + (1 - F0) * pow(max(1 - cos_theta, 0.f), 5);
}
...
inline float GTR2(const float n_dot_h, const float roughness) {
...
inline float GGX(const float n_dot_h, const float roughness) {
...
inline float smith_masking_gtr2(const float3 v_local, float roughness) {
    const float alpha = roughness * roughness;
    const float a2 = alpha * alpha;
...
inline float3 sample_visible_normals(float3 local_dir_in, const float alpha, const float2 rnd_param) {
    ...
    const float3 hemi_dir_in = normalize(float3(alpha * local_dir_in[0], alpha * local_dir_in[1], local_dir_in[2]));
    ...
    float3 N = normalize(float3(alpha * hemi_N[0], alpha * hemi_N[1], max(0.f, hemi_N[2])));
