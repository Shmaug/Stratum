...
void load_assimp(Node& root, CommandBuffer& commandBuffer, const fs::path& filename) {
	...
	if (scene->HasLights())
		printf_color(ConsoleColor::eYellow, "Warning: lights are unsupported\n");
	...
	if (scene->HasMaterials()) {
		...
		for (int i = 0; i < scene->mNumMaterials; i++) {
			...
			if (m->Get(AI_MATKEY_SHININESS, roughness.value) == AI_SUCCESS)
				roughness.value = sqrt(2 / (roughness.value + 2));
