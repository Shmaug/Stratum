...
public:
	...
	template<typename T, size_t N> inline byte_blob(const array<T,N>& cp) : mData(vector<char>(sizeof(T)*N)) { memcpy(data(), cp.data(), sizeof(T)*N); }
	inline byte_blob(size_t size, const void* data) : mData(vector<char>((const char*)data, (const char*)data + size)) {}
