...
void Profiler::DrawProfiler(GuiContext* gui) {
	Device* device = gui->Device();
	Font* font = device->AssetManager()->LoadFont("Assets/Fonts/FantasqueSansMono/FantasqueSansMono-Regular.ttf");
	...
	snprintf(buf, 128, "%u descriptor sets\t%u command buffers\t%u/%u allocations\t%.3f / %.3f mb (%.1f%%)",
